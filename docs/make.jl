using Documenter, SimulationParameters

# Call functions
open(joinpath(@__DIR__, "src/api.md"), "w") do f
    println(f, "# API Reference\n")
    for page in keys(SimulationParameters.document)
        if page == :Expressions
            continue
        end
        println(f, "## $page\n")
        println(f, "```@docs")
        for item in SimulationParameters.document[page]
            println(f, "$item")
        end
        println(f, "```")
    end
...
makedocs(;
    modules=[SimulationParameters],
    format=Documenter.HTML(),
    sitename="SimulationParameters",
    checkdocs=:none,
...
# https://projecttorreypines.github.io/SimulationParameters.jl/stable
# will be built from latest release tagged with a version number.
# The development documentation that will be hosted on
# https://projecttorreypines.github.io/SimulationParameters.jl/dev
...
deploydocs(;
    repo="github.com/ProjectTorreyPines/SimulationParameters.jl.git",
    target="build",
    branch="gh-pages",
    devbranch="master",
    versions=["stable" => "v^", "v#.#"]
